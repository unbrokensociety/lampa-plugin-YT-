/*!
 * Skip Intro/Outro для Lampa — v3.2
 * v3.2 — исправлен «script error»: весь код в защитной обёртке,
 * новый флаг загрузки (конфликт со старым плагином невозможен),
 * только ASCII-символы + кириллица, уведомление при старте.
 */
(function () {
  'use strict';

  if (window.__skipIntroV3Loaded) return;
  window.__skipIntroV3Loaded = true;
  window.__skipIntroLoaded = true;
  window.__skipIntroVersion = '3.2';
  ...
  try {
    ...
    function toast(msg) {
      try { Lampa.Toast.show({ title: '' + msg, time: 5000 }); } catch (e) {}
    }
    ...
    var S = {
      ...
      init: function () {
        ...
        p({ component: 'skip_intro', param: { name: 'skip_intro_debug', type: 'trigger', default: false },
          field: { name: 'Отладка', description: 'Показывать на экране, что нашёл плагин' } });
    ...
    var DC = {
      ...
      get: function (m) {
        ...
        var r = this.all()[k];
        if (!r) return null;
        if (!r._ts || Date.now() - r._ts > 30 * 24 * 3600 * 1000) { var o = this.all(); delete o[k]; this.save(o); return null; }
    ...
    var Sources = {
      ...
      load: function (meta) {
        ...
        if (cached) { log('базы: из кэша, сегментов:', cached.length); return Promise.resolve(cached); }
    ...
    var Subs = {
      ...
      parse: function (txt) {
        try {
          ...
          log('субтитры: файл загружен, реплик:', cues.length);
        ...
        return new Promise(function (resolve) {
          ...
          if (self.ctx.state === 'suspended') {
            ...
            self._wait = setInterval(function () {
              ...
              else if (tries > 10) { log('звук: AudioContext не активен'); done(null); }
            }, 300);
          } else connect();
        });
      },
    ...
    var C = {
      ...
      begin: function (v, src) {
        ...
        if (meta.isMovie && !S.movies()) { log('фильмы отключены'); return; }
      ...
      logSegs: function (from) {
        ...
        for (var i = 0; i < this.segs.length; i++) {
          s.push(this.segs[i].type + ' ' + this.segs[i].start + '-' + this.segs[i].end + 's');
        }
    ...
    toast('Пропуск заставок v3.2: загружен');

  } catch (e) {
    try { console.error('[SkipIntro] критическая ошибка:', e); } catch (e2) {}
    try { Lampa.Toast.show({ title: 'SkipIntro: ошибка — ' + (e && e.message ? e.message : e) }); } catch (e3) {}
  }
})();

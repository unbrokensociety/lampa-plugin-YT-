/*!
 * Skip Intro/Outro для Lampa — v3.1
 * v3.1 — исправлено «работает только тестовый режим»:
 *   1) анализ субтитров и звука стартует СРАЗУ, базы опрашиваются параллельно в фоне
 *      (раньше пустые/медленные API задерживали анализ — заставка уже проходила);
 *   2) метаданные: читаются и season/episode, и season_number/episode_number,
 *      ID эпизода больше не путается с ID сериала/фильма;
 *   3) субтитры ищутся в нескольких местах и с повторами;
 *   4) звук анализируется с первой секунды; AudioContext проверяется перед
 *      подключением, чтобы случайно не заглушить видео;
 *   5) сегменты из всех источников ОБЪЕДИНЯЮТСЯ (базы > субтитры > звук);
 *   6) диагностика: настройка «Отладка» (уведомления на экране) и
 *      команда __skipIntro.dump() в консоли.
 */
(function () {
  'use strict';
  ...
  function toast(msg) {
    try { Lampa.Toast && Lampa.Toast.show({ title: '' + msg, time: 6000 }); } catch (e) {}
  }
  ...
  var S = {
    ...
    init: function () {
      ...
      p({ component: 'skip_intro', param: { name: 'skip_intro_debug', type: 'trigger', default: false },
        field: { name: 'Отладка', description: 'Показывать на экране, что нашёл плагин (для проверки работы)' } });

      p({ component: 'skip_intro', param: { name: 'skip_intro_test', type: 'trigger', default: false },
        field: { name: 'Тестовый режим', description: 'Кнопка в начале ЛЮБОГО видео' } });

      p({ component: 'skip_intro', param: { name: 'skip_intro_auto', type: 'trigger', default: false },
        field: { name: 'Всегда автопропуск' } });

      p({ component: 'skip_intro', param: { name: 'skip_intro_detect', type: 'trigger', default: true },
        field: { name: 'Умное обнаружение', description: 'Анализ субтитров и звука, если нет данных в базах' } });

      p({ component: 'skip_intro', param: { name: 'skip_intro_movies', type: 'trigger', default: true },
        field: { name: 'Работать с фильмами' } });
    ...
    debug: function () { return this.f('skip_intro_debug', false) === true; },
  ...
  var DC = {
    ...
    get: function (m) {
      ...
      var r = o[k];
      if (!r) return null;
      if (!r._ts || Date.now() - r._ts > 30 * 24 * 3600 * 1000) { delete o[k]; this.save(o); return null; }
  ...
  /* ============ сеть: таймаут, повтор, при CORS — прокси Lampa ============ */
  ...
  var Net = {
    ...
    proxied: function (url) {
      try {
        var fn = window.Lampa && Lampa.Utils && (Lampa.Utils.proxyUrl || Lampa.Utils.proxy);
        if (typeof fn === 'function') {
          var r = fn(url);
          if (r && typeof r === 'string' && r !== url) return r;
        }
      } catch (e) {}
      return null;
    },
    ...
    get: function (url) {
      ...
      return go(1, 700).catch(function (e) {
        /* последняя попытка: через CORS-прокси Lampa, если он есть */
        if (e && (e.message === 'network' || e.message === 'timeout')) {
          var p = self.proxied(url);
          if (p) { log('повтор через прокси'); return self.once(p); }
        }
        throw e;
      });
    }
  ...
  var Sources = {
    ...
    load: function (meta) {
      ...
      if (cached) { log('базы: ответ из кэша, сегментов:', cached.length); return Promise.resolve(cached); }
      ...
      function run(i) {
        ...
        return fns[i](meta).then(
          ...
          function (err) { log('база', i, 'ошибка:', err && err.message); return run(i + 1); }
        );
      }
      return run(0).then(function (s) {
        if (ok) Cache.set(meta, s || []);
        log('базы: сегментов:', (s || []).length);
        return s || [];
      });
    }
  };
  ...
  var Subs = {
    /* Поиск субтитров сразу во всех местах: объект видео, плейлист,
       <track> элементы, встроенные дорожки */
    find: function (video, extra, cb) {
      var self = this;
      var urls = [];
      var lists = [];
      ...
      try {
        if (window.Lampa && Lampa.Player && typeof Lampa.Player.video === 'function') {
          var pv = Lampa.Player.video();
          if (pv && typeof pv === 'object' && !pv.nodeType) {
            if (pv.subtitles) lists.push(pv.subtitles);
            if (pv.customSubs) lists.push(pv.customSubs);
            if (pv.subtitle) lists.push(pv.subtitle);
          }
        }
      } catch (e) {}

      /* <track> внутри <video> — берём их файлы напрямую */
      try {
        if (video && video.querySelectorAll) {
          var trs = video.querySelectorAll('track');
          for (var i = 0; i < trs.length; i++) {
            var s = trs[i].getAttribute('src');
            if (s) urls.push(s);
          }
        }
      } catch (e) {}
      ...
      for (var L = 0; L < lists.length; L++) {
        var a = lists[L]; if (!a) continue;
        if (!Array.isArray(a)) a = [a];
        for (var j = 0; j < a.length; j++) if (a[j] && a[j].url) urls.push(a[j].url);
      }

      /* встроенные дорожки: если cues уже загружены — берём;
         если нет — включаем скрытый режим, чтобы браузер их загрузил */
      try {
        if (video && video.textTracks) {
          for (var t = 0; t < video.textTracks.length; t++) {
            var tt = video.textTracks[t];
            if (tt.cues && tt.cues.length > 4) {
              var cues = self.fromCues(tt.cues);
              if (cues) { log('субтитры: из дорожки плеера, реплик:', cues.length); cb(cues); return; }
            }
      ...
      function tryUrl(idx) {
        if (idx >= urls.length) { cb(null); return; }
        self.load(urls[idx], function (txt) {
          var cues = txt ? self.parse(txt) : null;
          if (cues) cb(cues);
          else tryUrl(idx + 1);
        });
      }
      if (urls.length) { tryUrl(0); return; }
      cb(null);
    },
    ...
    analyze: function (cues, dur, isMovie) {
      ...
      if (cues[0].start >= 15 && cues[0].start <= maxGap) {
        best = cues[0].start;
        intro = { start: 0, end: cues[0].start, confidence: 'medium' };
      }
      ...
      for (var i = 0; i < cues.length - 1; i++) {
        ...
        if (g >= 15 && g <= maxGap && g > best) {
          best = g;
          intro = { start: cues[i].end, end: cues[i + 1].start, confidence: 'medium' };
        }
      }

      /* заставка = серия музыкальных реплик (♪ ... ♪) */
      ...
      if (mus && intro && mus.start <= intro.end + 10 && mus.end >= intro.start - 10) {
        intro = { start: Math.min(intro.start, mus.start), end: Math.max(intro.end, mus.end), confidence: 'high' };
        mus = null;
      }
      /* у фильма пробел без подтверждения — скорее пролог без реплик */
  ...
  var AudioDet = {
    ...
    detect: function (video, isMovie) {
      ...
      return new Promise(function (resolve) {
        ...
        } catch (e) { self.dead = true; return done(null); }

        /* подключаемся только когда контекст активен — иначе можно заглушить видео */
        function connect() {
          try {
            ...
            self.dead = true;
            log('звук: подключение не удалось:', e && e.message);
            return done(null);
          }
          sample();
        }
        ...
        if (self.ctx.state === 'suspended') {
          ...
          var tries = 0;
          self._wait = setInterval(function () {
            tries++;
            if (self.ctx.state === 'running') { self.clear(); connect(); }
            else if (tries > 10) { log('звук: AudioContext не активен — анализ пропущен'); done(null); }
          }, 300);
        } else connect();
      });
    },
  ...
  var Button = {
    el: null, visible: false, _iv: null, _last: 0,
  ...
  var Meta = {
    resolve: function (payload) {
      ...
      try {
        if (window.Lampa && Lampa.Player) {
          ...
          if (pl && pl.length) {
            ...
            var it = typeof pi === 'number' ? (pl[pi] || null) : (pi && typeof pi === 'object' ? pi : null);
            if (it) cands.push(it);
          }
          if (typeof Lampa.Player.video === 'function') {
            var pv = Lampa.Player.video();
            if (pv && typeof pv === 'object' && !pv.nodeType) cands.push(pv);
          }
        }
      } catch (e) {}
      try {
      ...
      log('мета:', meta.isMovie ? 'фильм' : ('S' + meta.season + 'E' + meta.episode),
        '| tmdb:', meta.tmdbId, '| imdb:', meta.imdbId, '| субтитров:', meta.subs.length,
        meta.trackable ? '' : '| ⚠ ID НЕ НАЙДЕНЫ');
      return meta;
    },
    ...
    scan: function (node, depth, r, seen) {
      ...
      /* ВАЖНО: season/episode И season_number/episode_number — Lampa использует оба варианта */
      if (r.season == null) { var s = num(node.season != null ? node.season : node.season_number); if (s != null) r.season = s; }
      if (r.episode == null) { var ep = num(node.episode != null ? node.episode : node.episode_number); if (ep != null) r.episode = ep; }
      ...
      /* карточка фильма/сериала (но НЕ эпизода — у эпизода свой id, он нам не нужен) */
      var episodeLike = node.episode_number != null || node.season_number != null || node.still_path != null;
      if (r.cardId == null && node.id != null && !node.url && !episodeLike &&
  ...
  var C = {
    ...
    audioStarted: false, cues: null, reAnalyzed: false,
    ...
    reset: function () {
      ...
      this.audioStarted = false; this.cues = null; this.reAnalyzed = false;
      Button.hide();
      AudioDet.stop();
    },
    ...
    tick: function () {
      try {
        ...
        /* тот же элемент, но сменился src при большой позиции — это смена качества, не тревожим */
        if (v === this.video && src && this.src && src !== this.src && v.currentTime > 60) {
          this.src = src;
          return;
        }
    ...
    begin: function (v, src) {
      ...
      this.activeKey = null; this.shownAuto = null;
      this.audioStarted = false; this.cues = null; this.reAnalyzed = false;
      Button.hide(); AudioDet.stop();

      var meta = Meta.resolve(this.payload);
      this.meta = meta;

      if (S.debug()) {
        toast('SkipIntro: ' + (meta.isMovie ? 'фильм' : 'S' + meta.season + 'E' + meta.episode) +
          ' | tmdb: ' + meta.tmdbId + (meta.imdbId ? ' | imdb: ' + meta.imdbId : '') +
          (meta.trackable ? '' : ' | ID не найдены'));
      }
      ...
      if (S.test()) {
        ...
        log('ТЕСТОВЫЙ РЕЖИМ: кнопка с 3-й секунды');
        return;
      }

      /* сразу подставляем прошлую детекцию (если была) */
      var pre = meta.trackable ? (DC.get(meta) || []) : [];
      if (pre.length) {
        this.segs = this.clean(pre, 0);
        log('из кэша прошлой детекции:', this.segs.length);
      }

      /* ГЛАВНОЕ ИЗМЕНЕНИЕ: детекция сразу, базы параллельно */
      this.startDetection();
      this.loadApi();
    },

    /* ---------- детекция: субтитры (с повторами) + звук (сразу) ---------- */

    startDetection: function () {
      if (!S.detect() || !this.video || !this.meta) return;
      var self = this, v = this.video, meta = this.meta;
      log('умная детекция запущена (субтитры + звук)');

      var state = { subsDone: false, audioDone: false };
      function summary() {
        if (state.subsDone && state.audioDone && !self.segs.length) {
          log('итог: сегменты не найдены (базы пусты, субтитры недоступны, звук молчит)');
        }
      }

      /* звук — параллельно, с первой секунды */
      if (!this.audioStarted && !this.hasType('intro')) {
        this.audioStarted = true;
        AudioDet.detect(v, meta.isMovie).then(function (seg) {
          state.audioDone = true; summary();
          if (self.video !== v || !seg) return;
          log('звук: заставка', rnd(seg.start) + '–' + rnd(seg.end));
          self.mergeSegs([seg]);
        });
      } else {
        this.audioStarted = true;
      ...
      /* субтитры — сразу и с повторами (дорожки подгружаются позже старта) */
      var delays = [0, 6000, 15000, 35000];
      function attempt(i) {
        if (self.video !== v) return;
        Subs.find(v, meta.subs && meta.subs.length ? meta.subs : null, function (cues) {
          if (self.video !== v) return;
          if (cues) {
            state.subsDone = true;
            self.cues = cues;
            var segs = Subs.analyze(cues, v.duration || 0, meta.isMovie);
            log('субтитры: сегментов', segs.length);
            if (segs.length) self.mergeSegs(segs);
            summary();
          } else if (i + 1 < delays.length) {
            setTimeout(function () { attempt(i + 1); }, delays[i + 1]);
          } else {
            state.subsDone = true;
            log('субтитры недоступны');
            summary();
          }
        });
      }
    ...
    /* субтитры могли прийти раньше, чем стала известна длительность — переанализ */
    reAnalyze: function () {
      if (this.reAnalyzed || !this.cues || !this.video || !this.meta) return;
      var d = this.video.duration || 0;
      if (d > 0) {
        this.reAnalyzed = true;
        var segs = Subs.analyze(this.cues, d, this.meta.isMovie);
        if (segs.length) this.mergeSegs(segs);
      }
    },

    /* ---------- базы (в фоне, параллельно детекции) ---------- */

    loadApi: function () {
      var self = this, v = this.video, meta = this.meta;
      if (!meta || !meta.trackable) { log('ID не найдены — работаем только на детекции'); return; }

      Sources.load(meta).then(function (segs) {
        if (self.meta !== meta || self.video !== v) return;
        var dc = meta.trackable ? (DC.get(meta) || []) : [];
        var all = (segs || []).concat(dc);
        if (all.length) self.applyApi(all, 'база');
        else log('базы и кэш пусты — надеемся на детекцию');
      });
    },

    /* сегменты из баз имеют приоритет над детекцией */
    applyApi: function (segs, from) {
      var add = this.clean(segs, this.video ? this.video.duration || 0 : 0);
      if (!add.length) return;

      for (var i = 0; i < add.length; i++) {
        var n = add[i];
        for (var j = this.segs.length - 1; j >= 0; j--) {
          var e = this.segs[j];
          if (e.type === n.type && Math.min(e.end, n.end) - Math.max(e.start, n.start) > -10) {
            this.segs.splice(j, 1);
          }
        }
        this.segs.push(n);
      }
      ...
      if (this.meta && this.meta.trackable) DC.set(this.meta, this.segs);
      if (this.hasType('intro')) AudioDet.stop();
      this.logSegs(from || 'база');
      if (S.debug()) toast('SkipIntro: найдено сегментов — ' + this.segs.length);
    },

    /* объединение результатов детекции */
    mergeSegs: function (segs) {
      ...
      if (changed) {
        ...
        if (this.hasType('intro')) AudioDet.stop();
        this.logSegs('детекция');
      }
    },

    logSegs: function (from) {
      var s = [];
      for (var i = 0; i < this.segs.length; i++) {
        s.push(this.segs[i].type + ' ' + this.segs[i].start + '–' + this.segs[i].end + 'с');
      }
      log('сегменты (' + from + '):', s.join(', ') || 'нет');
    },
    ...
    /* ---------- диагностика из консоли: __skipIntro.dump() ---------- */
    dump: function () {
      var info = {
        version: window.__skipIntroVersion,
        video: !!this.video,
        currentTime: this.video ? this.video.currentTime : null,
        duration: this.video ? this.video.duration : null,
        meta: this.meta,
        segments: this.segs,
        payloadKeys: this.payload && typeof this.payload === 'object'
          ? Object.keys(this.payload).slice(0, 30) : this.payload
      };
      console.log('[SkipIntro] dump:', info);
      return info;
    }
  };
